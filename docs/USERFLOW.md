...
This document details the step-by-step user flows, interactions, and system state transitions across all actors (Exercise Authors, Students/Candidates, and Instructors/Admins) for both online (server-backed) and offline (drive-backed) environments in TDES.

---

## System Overview

TDES is designed to orchestrate, distribute, and grade coding labs securely. The system operates in two modes:
1. **Online (Registry Server-Backed) Mode**: The CLI communicates directly with a central HTTP server to pull exercises, submit solutions, verify PIN-based authentication, and fetch grades.
2. **Offline (Drive-Backed) Mode**: The CLI reads from and writes encrypted packages to physical drives (e.g. USB flash drives), which the instructor collects and evaluates in bulk.

```mermaid
graph TD
    Author[Exercise Author] -->|Publish| Server[TDES Registry Server]
    Instructor[Instructor / Admin] -->|Onboard Roster / Fetch Grades| Server
    Student[Student / Candidate] -->|Fetch / Submit / TOFU| Server
    Student -->|Submit Encrypted Envelope| Drive[(Physical USB Drive)]
    Drive -->|Batch Evaluate| Instructor
```
...
sequenceDiagram
    ...
    Server->>DB: Save exercise metadata & versions
    Server->>Server: Store public & private artifacts to disk
    Server-->>CLI: Return 201 Created (version metadata)
    deactivate Server
    CLI-->>Author: Display success & checksums
    deactivate CLI
```
...
sequenceDiagram
    ...
    Server-->>CLI: Return exercise metadata & public artifact SHA-256
...
sequenceDiagram
    ...
    Note over CLI: Derives shared secret & encrypts submission via AES-GCM
    ...
    loop For each decrypted student package
        ...
        Evaluator-->>CLI: Returns test outcomes & points
    end
